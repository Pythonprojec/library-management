...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:Joker4sure@localhost:5432/libraryDB"
...
def AddSubject():
    ...
    for itm in SubList:
        tabl = tabl+"<TR ><TD>"+str(itm.subID) + \
            "</TD><TD>"+itm.subName+"</TD></TR>"
    tabl = tabl+"</Table></DIV>"
...
def saveBook():
    ...
    bookMas = bookMaster(accNumber=aNo, bookTitle=bNam, SubID=sID,
                         authorName=aNam, PublisherName=pNam, pages=pag, price=prc, status='A')
...
def Books():
    ...
    tabl = '''<div div class="col-md-12">
            ...
            <TD>Subject</TD></TR>'''
    for itm in bList:
        tabl = tabl+"<TR ><TD>"+itm.accNumber+"</TD><TD>" + \
            itm.bookTitle+"</TD><TD>"+itm.subject.subName+"</TD></TR>"
    tabl = tabl+"</TABLE></DIV>"
    ...
    for itm in sList:
        dd = dd+"<option value=\"" + \
            str(itm.subID)+"\">"+itm.subName + "</option>"
    dd = dd + '''</select>
</div>'''
...
def users():
    ...
    for itm in uList:
        tabl = tabl+"<TR ><TD>" + \
            str(itm.userID)+"</TD><TD>"+itm.userName + \
            "</TD><TD>"+itm.userType+"</TD></TR>"
    tabl = tabl+"</table></div>"
...
def saveIssue():
    ...
    # TODO: here is the error issue book generating
    issDet = IssueReturn(AccNumber=ano, userID=uid,
                         IssueDate=idt, ExpRetDate=exdt)
...
def BookIssue():
    ...
    for itm in bList:
        bdd = bdd+"<option value=\"" + \
            str(itm.accNumber)+"\">"+itm.bookTitle + "</option>"
    bdd = bdd + '''</select>
</div>'''
    ...
    for itm in uList:
        udd = udd+"<option value=\"" + \
            str(itm.userID)+"\">"+str(itm.userID) + \
            "."+itm.userName + "</option>"
    udd = udd + '''</select>
</div>'''
...
def BookReturn():
    ...
    for itm in iList:
        bdd = bdd+"<option value=\"" + \
            str(itm.transID)+"\">"+itm.book.bookTitle + "</option>"
    bdd = bdd + '''</select>
</div>'''
...
def BookList():
    ...
    for itm in uBook:
        ...
        tabl = tabl+"<TR ><td>"+itm.book.bookTitle+"</TD><TD>"+str(itm.IssueDate)+"</TD><TD>"+str(
            itm.ExpRetDate)+"</TD><TD>"+str(itm.ActRetDate)+"</TD><TD>"+str(itm.OverdueDays)+"</TD><TD>"+str(fine)+"</TD></TR>"
    tabl = tabl+"</table>"
...
def display():
    ...
    if styp == "T":
        lBook = db.session.query(bookMaster).filter(
            bookMaster.bookTitle.ilike(look_for))
    if styp == "A":
        lBook = db.session.query(bookMaster).filter(
            bookMaster.authorName.ilike(look_for))
    if styp == 'S':
    ...
    tabl = '''<table class="table table-bordered border-success" ><TR style="text-weight:bold; background-color:#ccffcc;">
            ...
            <TD>Pages</TD></TR>'''
    for itm in lBook:
        tabl = tabl+"<TR ><td>"+itm.accNumber+"</TD><TD>"+itm.bookTitle+"</TD><TD>"+itm.authorName + \
            "</TD><TD>"+itm.PublisherName+"</TD><TD>" + \
            str(itm.price)+"</TD><TD>"+str(itm.pages)+"</TD></TR>"
    tabl = tabl+"</table>"
    strg = '''<div class="text-center"><h5>Searched Books for "''' + \
        skw+'''"</h5></div><hr>'''+tabl
    text = text = Markup(strg)
    return render_template("dashboard.html", text=text)
